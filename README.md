...
하드웨어 성능이 빠르게 향상되면서, 동기와 비동기 처리의 속도 차이가 사람의 눈으로는 거의 구분되지 않는 환경이 되었습니다.

개념을 설명으로만 이해하는 것과, 직접 눈으로 보면서 이해하는 것은 다르다고 생각했습니다. 그래서 **의도적으로 800ms 지연(setTimeout)을 추가**해 동기는 순서대로 하나씩, 비동기는 병렬로 동시에 처리되는 흐름을 시각화했습니다.

포켓몬이라는 친숙한 소재와 진화 가족 단위 구성을 선택한 것도 같은 맥락입니다. 개념 학습에서 소재의 낯섦이 불필요한 인지 부하를 만든다고 생각해, 최대한 직관적인 맥락을 사용했습니다.
...
코드가 동작하는 결과(카드 렌더링, 타이머)는 보였지만, 내부에서 어떤 순서로 실행되는지는 보이지 않았습니다. `setTimeout`이 왜 있는지, `await` 이후 코드가 언제 재개되는지를 코드만 읽어서는 추적하기 어려웠습니다.
